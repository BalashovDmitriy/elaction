...
1. **Типы везде.** Аргументы, возвращаемые значения, поля. Автоматически это не
   проверяется: правила на типы у `gdlint` нет, а предупреждения движка о
   нетипизированном коде в проекте не включены — держится на авторе и ревью.
