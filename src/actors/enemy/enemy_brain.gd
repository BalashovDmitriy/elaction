...
## пункт 2): от высокой пули он уходит на колено, от низкой — ложится. В кабинах
## он ездит пассажиром (ADR-0025, решение 6), но это решает [Enemy] с уровнем,
## а не мозг. Не прыгает — ни в оригинале, ни здесь (ADR-0026).
...
## Дальше этого агент не стреляет, м.
var fire_range: float = 6.0
...
## Разворачивает агента. Зовёт узел, когда пол впереди кончился, а цели не
## видно: слепой агент патрулирует этаж, а не стоит у края (ADR-0023).
...
## в кого целиться: жив ли Otto и виден ли. Невидимого — в тени или за дверью —
## мозг не преследует и не обстреливает: идёт, куда шёл (ADR-0023, решение 8).
##
