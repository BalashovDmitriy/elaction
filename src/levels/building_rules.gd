...
##
## Два — это уже лифт, но только для одноярусной кабины. В M18b приходит
## двухэтажная — жёсткая пара ярусов ([ADR-0024]): в шахте на два этажа она
## заняла бы её целиком и не сдвинулась бы ни разу, то есть вернула бы тот же
## неподвижный лифт, только на более редком сиде. Поэтому минимум три.
##
## Правило нужно и раскладке, и проверкам, поэтому живёт числом, а не двумя
## одинаковыми константами в разных файлах.
const MIN_SHAFT_FLOORS: int = 3
