...
##
## Ширина выводится из шага места: шестнадцать шагов по 1.8 м и два отступа.
## Шаг — с кадра оригинала, 24 px из 40 просвета (ADR-0026, решение 3).
@export var width: float = 33.6
...
@export var shaft_width: float = 1.8
...
##
## Дальний край — там, где макушка пассажира проходит низ плиты, плюс полкорпуса.
## До 2.7 м от площадки ему уходить нельзя: дальше начинается соседнее место,
## а в нём может стоять шахта.
@export var escalator_gap_offset: float = 0.12
@export var escalator_gap_width: float = 1.98

## На сколько эскалатор уводит в сторону, спускаясь на этаж, м.
##
## В свои два места: нижняя площадка с пассажиром на ней не заходит за 2.7 м —
## в соседнее место, где на этаже ниже может быть проём шахты. Вход–выход ~58°
## против ~56° у оригинала, полотно ~66° против ~63° (ADR-0026, решение 6).
@export var escalator_run: float = 2.24
...
##
## Колено — с кадра оригинала: присевший агент там 14 px над полом из 22–23, и
## высокая пуля Otto (15 px) проходит над ним впритирку. Лёжа — та же доля
## стойки, что была до M18c (ADR-0026, решение 2).
@export var agent_kneel_height: float = 1.0
@export var agent_prone_height: float = 0.52
...
func _shafts_wanted(index: int) -> int:
	...
	var last := floors - MIN_SHAFT_FLOORS
