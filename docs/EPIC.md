...
Пройдены M0–M20, все влиты в `main`. Идёт **M21** — модели Otto, агентов и
машины из паков Quaternius. Дальше **M21b** — этажи, крыша и лифт моделями паков,
**M22** — грейдинг и полировка, **M23** — онлайн-лидерборд (опционально).
...
- [ ] Пайплайн: исходник пака в `assets/source/`, `build_actors.py` приводит рост,
      палитру, клипы; федора и очки агенту, пистолет обоим
- [ ] Анимация смешанная: клипы пака (стойка, ходьба, выстрел, смерть) и позы
      кодом с коленями и локтями (присед, лёжа, прыжок, удар, раздавленный)
- [ ] Присед на корточках и лёжа по высотам ROM (долг M18c)
- [ ] Заземление по крайним вершинам костей — модель в двести раз тяжелее
- [ ] Машина у выхода из Cars Pack: пять моделей без такси и полиции, жребий
      по соли, первое здание — красная спортивная; длина — `CAR_LENGTH`
- [ ] Инструмент кадра поз `tools/actor_shot.tscn`
- [ ] Пуля — трассер с хвостом и вспышкой у ствола вместо коробки (замечание
      пользователя по ходу вехи)
- [ ] Звонка лифта на этажах нет (решение пользователя: «как будто в звонок
      звонят»)
- [ ] Тесты поз по вершинам и габаритам, клипы и кости в моделях, жребий машины
...
Решения пользователя (2026-09-24), свой ADR на старте вехи. Замечание: «на
этажах одни квадраты непонятные», «всё натурально, как в жизни».

- [ ] Тип здания — жребий по соли: отель или офисная башня, у каждого своя
      вывеска и свой набор обстановки
- [ ] Обстановка этажей моделями паков: Ultimate House Interior (CC0), The Office
      Pack (CC0 и CC-BY 3.0 — авторы в CREDITS и README, тест на строку у
      каждой модели)
- [ ] Крыша детальнее
- [ ] Вывеска здания — вертикальная на углу фасада вдоль верхних этажей: её не
      закрывают ни техника крыши, ни верх кадра
- [ ] Лифт детальнее; у каждого портала — кнопки вызова «вверх/вниз» и табло
      с этажом, где кабина сейчас
- [ ] Шахта выделяется на фоне этажей: свои материалы с текстурами (металл,
      бетон, решётка) — CC0 с ambientCG и Poly Haven
- [ ] Тесты: у каждой модели лицензия и автор, обстановка без тел, табло
      показывает этаж кабины

**DoD:** этаж читается отелем или конторой без подписи, у лифта видно, где кабина.

### M22 · Грейдинг и полировка

Решения — [ADR-0030](adr/0030-grading-and-quality.md). Последняя веха пивота. Часть
сделана ещё в ветке M20 и здесь доводится по кадрам на детальной сцене.

- [ ] Тон — ночной нуар по референсу: холодные тени, тёплые лампы, глубже чёрный;
      игровые знаки остаются яркими. Кривые в коде с M20, подбор — по кадрам
...
- [x] Качество графики: низкое, среднее, высокое — в настройках, сразу (в ветке M20)
- [ ] Уровень «Ультра» (решение пользователя 2026-09-24, вместо трассировки лучей):
      SSR — отражения неона и ламп в полу и стекле, SDFGI/SSIL — отражённый
      свет, объёмный свет ламп. Настоящего RT в Godot 4.7 нет — только
      низкоуровневый Vulkan RT в `RenderingDevice`; опцией он станет, когда
      появится в основной ветке движка
...
- [ ] Уборка 2D: `tools/palette.py`, поля палитры без читателей
      (ADR-0019, решение 8)
