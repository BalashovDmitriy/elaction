...
Пройдены M0–M19, все влиты в `main`. Идёт **M20** — детализация сцены. Дальше
**M21** — модели Otto и агентов, **M22** — грейдинг и полировка, **M23** —
онлайн-лидерборд (опционально).
...
Решение пользователя: Quaternius Ultimate Modular Men Pack (CC0) — Business Man
для Otto и для агентов (тёмный костюм, голова со шляпой). Свой ADR на старте вехи.

- [ ] Модели из пака: сборка частей, риг, нормали под обводку, 1.68 м
- [ ] Колени и локти в позах: присед и лёжа по высотам ROM (долг M18c — присед
      на корточках)
...
**DoD:** Otto и агенты — люди в костюмах, а не коробки, и стойки держат высоты
пуль ROM.
