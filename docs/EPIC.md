...
**M23** — звук (PR #44), **M24a** (PR #45) и **M24b** (PR #47). Идёт **M24c** —
анимация и клавиши, затем **M24d** — добивания вместо удара ногой, **M24e** —
демо-режим и **M25** — онлайн-лидерборд (опционально).
...
- [ ] Кандидаты на слух: страница с плеером, по каждому слоту два–четыре
      файла CC0/CC-BY с автором и лицензией; выбирает пользователь
- [ ] Треки нуар-джаза: здание, тревога, меню, конец партии; джинглы —
      документ, жизнь, бонус, смерть, конец партии
- [ ] Эффекты тех же событий, что сейчас: шаги по ковру и бетону, выстрел,
      попадание, удар ногой, двери, кабина, эскалатор, лампы, смерти, машина,
      звуки меню
- [ ] Фон: улица и дождь — на крыше, у выхода и в меню в полную силу, на этажах
      глухо (шина `Ambience`, фильтр); гул шахты; гром к молниям с задержкой по
      дальности разряда
- [ ] Музыка: тревога наплывом, за красной дверью и на паузе глуше, джингл
      приглушает трек
- [ ] Синтез удалён: `render_audio.py`, `audio_dsp.py`; авторы в `CREDITS.md`
...
- [ ] Кабина, подъехавшая к этажу, не застревает вместе с Otto
- [ ] Верх и низ шахты не мерцают
- [ ] Дождь заново: капли гаснут о крышу, брызги, лужи, мокрая крыша, слои в городе
- [ ] Фон не ползёт полосами, когда камера едет
- [ ] Пули втрое быстрее у обеих сторон; замах и луч прицела агента; новый вид
- [ ] Трупы агентов лежат до конца здания
- [ ] Падение больше чем на этаж убивает везде
- [ ] Документов в здании 5–10 жребием
- [ ] Экран управления с паузы; номер здания в HUD
...
- [ ] Переходы между позами по времени, а не одной экспонентой; разворот телом
- [ ] Короткие паузы разворота и приземления в правилах; тест боя в пороге
- [ ] Агенты на тех же клипах
- [ ] Переназначение: одна клавиша и одна кнопка на действие, обмен при
      занятой, сброс, схема в `settings.cfg`
- [ ] Прогрев шейдеров: Shader Baker в экспорте, первый показ редких эффектов
...
- [ ] Выстрел вплотную — добивание; сзади — удушение или свёрнутая шея (300),
      спереди — серия ударов или рукоятью (200), +100 в темноте и на 11–15
- [ ] Напрыгивание сверху — само, при приземлении на агента (300)
- [ ] Замедление мира на время сценки, Otto уязвим
- [ ] Клипы UAL 2, режиссёр парной сценки, правила с тестами, бот добивает

### M24e · Демо-режим

Автомат между партиями крутит игру сам. Меню остаётся как есть (решение
пользователя): демо включается поверх него по бездействию и уходит с любой
клавиши. Otto ведёт бот тестов.

- [ ] Сверка с оригиналом: когда включается, сколько длится, что показывает
- [ ] Бот в игре, здание по сиду, выход в меню
...
| 4 | Не подтверждены ни ROM, ни источниками: кабина игрока встаёт между этажами (ADR-0004, пункт 6); возвращение в игру — неуязвимость 1.5 с и место подальше от агентов; безопасное расстояние двери 2.88 м (`AGENT_SAFE_RELEASE`) | — |
| 9 | Уход отставшего агента на двадцатом этаже: ROM его запрещает, у нас это условие не взято — с ним сид 2 стоил 10 смертей вместо одной | баланс боя |
...
Releases ([ADR-0013](adr/0013-release-and-versioning.md)); 6 и остальное из 4 — сверка
