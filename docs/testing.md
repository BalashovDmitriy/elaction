...
одном замере, а не на догадке, — его делает `python tools/test_times.py --by-test`:
...
единицы на N частей той же жадной раскладкой по `KNOWN_SLOW` и гонит K-ю, а на
машине она снова раскладывается по ядрам. Части выходят ровными — около 290
условных секунд каждая, — и машина идёт примерно столько же, сколько самый
долгий сид. Импорт ресурсов каждая машина делает сама: без `.godot/imported`
тесты не загрузят ни одной сцены.

## Долгий прогон смотрят по ходу

Даже шардами набор идёт около трёх минут, а один сид бота на настоящем здании —
полторы-две. Ждать конца, чтобы узнать, что
