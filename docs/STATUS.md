...
| **Состояние** | Раскладка готова, бот проходит все пять сидов, бой вернулся к полной проверке — мерой стало число смертей. **Осталось: авторевью, `check.ps1`, скриншоты, PR** |
...
[M18c](EPIC.md). Раскладка мержится раньше: она готова и проверена, а баланс —
своя ось.
...
- **`_moves` и `_links` в `BuildingRoute` — два счёта одного графа.** Слить их
  ...
  эскалатора `_links` ругается, `_moves` молча пропускает.

### Дальше

**Авторевью и `check.ps1` были отложены сознательно** (решено 2026-09-22): веха
собиралась и пушилась без них, чтобы не держать раскладку в ветке. Ритуал
закрытия вехи требует и того и другого ([CLAUDE.md](../CLAUDE.md), «Завершение
вехи»), и пропущены они были не потому, что не нужны.

- [x] `/code-review xhigh --fix` — веха переписала раскладку, граф и бота разом,
      и правок в ней больше, чем в любой прошлой
- [ ] `tools/check.ps1` целиком после правок ревью

Потом PR на M18a. Дальше M18b (эскалатор конструкцией, столб света в шахте,
двухэтажная кабина) и M18c (баланс).
...
- **Бой заучивается: в нём нет случайности вовсе.** Во всём `src/` генератор
  ...
  **Правится в M18c** — там же, где остальные числа боя. Случайность должна
...
- **Прогон с боем на сиде 1 не повторяется.** Сиды 2 и 3 воспроизводятся от
  ...
  к худшему: закреплять в проверке шум нельзя. **Разобрать до M18c** — на нём
  стоят все замеры баланса боя.
