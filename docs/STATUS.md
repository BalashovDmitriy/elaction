...
- `src/levels/env_textures.gd` — сборка и кэш текстур; список ассетов, которые просит
  уровень, лежит там же и служит источником правды для теста.
- **Тесты (178, было 173):** у каждого ассета из списка есть все три карты, они одного
  размера, тайл собирается с нормалью и бликом, толщина тайла равна толщине перекрытия
  из правил здания, и собранное здание действительно кладёт ассет, а не заливку.
...
1. Задняя стена, окна и рама, тайл боковой стены здания.
2. Двери — обычная и красная, кабина лифта, эскалатор, лампа, выход.
3. Город за окнами получает текстуры, правила раскладки из M6 остаются.
4. Скриншоты вехи и авторевью.
## Принятые решения
