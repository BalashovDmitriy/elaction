...
| **Текущая веха** | M24c — анимация и клавиши |
| **Ветка** | `feat/m24c-animation` |
| **Состояние** | Сверка и вопросы пройдены, ADR-0039; начат перенос клипов UAL |
...
- [ ] Клипы UAL на скелете пака: перенос в `build_actors.py`, прыжок тремя
      фазами, приземление, присед, стойка, ходьба, выстрел, смерть.
      Перенос готов: `build_actors.py ual <glb>` урезает UAL до 492 КБ
      (`assets/source/quaternius/ual_clips.glb`), `_retarget_ual` переносит
      повороты в мире (арматура пака повёрнута импортёром на 90° вокруг X),
      стопы ставит на голень, ход таза — на `Body`, и каждый кадр заземляет
      по вершинам: ноги пака длиннее, и в шаге подошва уходила в пол на 6 см.
      Основа поз кодом — нейтральная стойка `stand` (UAL `Idle_Loop`): от
      стойки с пистолетом в двух руках углы рук поехали, и залёгший вышел над
      пулей. Суффикс `_loop` Godot срезает с имени клипа — клип полёта
      назван `jump_air`. Тесты рига и поз зелёные; в игре клипы прыжка ещё
      не показываются
- [ ] Переходы между позами по времени; разворот телом
- [ ] Паузы разворота и приземления в правилах; тест боя в пороге
- [ ] Агенты на тех же клипах
- [ ] Переназначение клавиш: слот клавиатуры и слот геймпада, обмен, сброс,
      схема в `settings.cfg`
- [ ] Прогрев шейдеров: Shader Baker и невидимый первый показ эффектов
- [ ] Кадры, авторевью, `check.ps1`, README
- [ ] PR

## Что дальше

- **M24d — демо-режим:** автомат между партиями крутит игру сам; включается по
  бездействию поверх меню, Otto ведёт бот тестов.
