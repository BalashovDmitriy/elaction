...
| **Состояние** | Сверка проведена, вопросы закрыты, ADR-0011 принят, Blender 5.2.1 поставлен; код вехи впереди |
...
- **Blender 5.2.1 LTS поставлен** через winget, проверен headless: `bpy 5.2.1`,
  ...
  winget не кладёт его в `Links`, значит поиск придётся вести и по Program Files.
- **ADR-0011 написан**, эпик разделён на M7a и M7b, вопрос «согласованность кадров
  анимации» из открытых снят.

### Что дальше по порядку

1. `tools/blender_bin.py` — поиск Blender по образцу `tools/godot_bin.py`.
2. Пайплайн целиком на одном дешёвом ассете: рендер → импорт-пресет → `CanvasTexture` →
   свет из M6 на нём ложится. Пока это не проверено на чём-то одном, остальное не начинаем.
3. `tools/palette.py` и генератор окружения `tools/render_env.py`.
4. Тайлсет, перекрытия, задняя стена, окна; двери, кабина, эскалатор, лампа, выход.
5. Город за окнами получает текстуры, правила раскладки из M6 остаются.
6. Тесты: каждый ассет, который просит сцена, существует, делится на кадры, имеет нормаль.
## Принятые решения
