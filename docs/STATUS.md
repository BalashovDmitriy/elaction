...
- [ ] Красная дверь: закрывается, 4,73 с, раньше не выйти, документ на выходе,
      глухо сквозь дверь, агенты иногда ждут у двери. Шина SFX получила свой
      фильтр «из-за стены» (`Sounds.muffle_world`)
...
- [x] Выход: подошёл — сел — фары — уехал; бонус поверх, затемнение. Садятся
      ...
      бонус набегает на HUD, смена здания под `FadeCurtain`
- [x] Не все документы: лифт не везёт в подвал без всех документов — нижняя
      остановка заперта, проём над подвалом закрыт люком (`BasementLock`);
      возврат к пропущенной двери снят вместе с `DocumentRoute`
