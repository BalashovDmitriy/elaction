...
| **Текущая задача** | Долг после сверки документации |
| **Ветка** | `fix/sweep-debt` поверх `docs/sweep` |
| **Состояние** | Долг закрыт, `check.ps1` зелёный. **Осталось: PR** (после PR `docs/sweep`) |
...
## Текущая задача — долг после сверки
