...
- `Lamp` — подвес под потолком. Висит так, что стоя в неё не попасть: выстрел стоя
  идёт в 20 px над полом, а подвес начинается выше. Сбить её можно в прыжке.
...
- Агент на тёмном этаже видит вчетверо ближе. Это одно число в `EnemyBrain`,
  и его же будет удобно ослаблять в поздних зданиях.
...
Проверено кадрами: выстрел по лампе в прыжке и погасший этаж, падение лампы на
агента с начислением 300 очков, целость эскалатора после выключения форм.

Не проверено кадрами: дальность агента в темноте и сама неуязвимость на эскалаторе —
в кадр они не попадают. Первое проверено тестом на уровне правил, второе следует из
того, что у выключенных форм пуле не во что попадать.
