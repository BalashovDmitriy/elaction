...
| **Ветка** | `docs/sweep` |
| **Состояние** | Сверка сделана, `check.ps1` зелёный. **Осталось: PR** |
...
- **Архив релиза без лицензий — до первого тега обязательно.** `tools/package.py`
  кладёт рядом с игрой только `LICENSE` и лицензию Pixellari. Нет текста OFL
  шрифта Exo 2 (HUD с M22) и нет `CREDITS.md`, а модели под CC-BY 3.0 требуют
  указать авторов там, где их распространяют. Найдено сверкой документации.
- **Строгую типизацию никто не проверяет.** Правило 1 CLAUDE.md держалось на
  «это проверяет `gdlint`», но правила на типы у `gdlint` нет, а предупреждения
  движка о нетипизированном коде в `project.godot` не включены. Лечится
  `debug/gdscript/warnings/untyped_declaration` уровнем «ошибка» — тогда ловит
  `godot_check.py`; сначала придётся дотипизировать то, что всплывёт.
- **Бонус за здание без потолка.** В ROM `1000 × min(10, здание)`
  ([`arcade-rom.md`](reference/arcade-rom.md), @5793), у нас в `GameState` —
  `BUILDING_BONUS * building` без ограничения.
- **Устаревшие комментарии в коде** (сверка документации):
  - `escalator.gd:7` и `building_decks.gd:84` ссылаются на «ADR-0005, пункт 8»,
    а эскалатор — ADR-0004, пункт 8;
  - `screenshotter.gd` (строки 36, 109, 415) называет `tools/light_shot.tscn`,
    которого нет, — по смыслу `dark_shot.tscn`;
  - `run_tests.py`: комментарии у `SHARDS_MAX` и `SPLIT_BY_TEST` описывают
    времена до разрезания дорогих тестов по сидам;
  - `menu.gd:393` — «переназначение отложено до M9»;
  - шапка `greybox_level.gd` — «серые коробки, модели придут вехами M16–M19»;
  - `test_proportions.gd` обещает «окна вернутся в M19 вместе со своей
    проверкой» — окон в стенах здания нет;
  - `requirements-assets.txt` не говорит, что Pillow нужен `build_textures.py`.
- **Агент уходит в дверь проще, чем в ROM:** отрыв в два этажа
  (`AGENT_FAR_FLOORS`), а в ROM — 80 px (≈1.7 этажа) и условия «этаж ≥ 8 и не
  20». Похоже на сознательное упрощение, но в ADR не записано.
- **Трассировка лучей** (вопрос пользователя, 2026-09-24). В Godot 4.7 только
  ...
  движка.
- **Матрица джоб в CI не заведена.** Тесты в CI идут одним джобом `engine`:
  `run_tests.py` делит набор на шарды внутри раннера ([`testing.md`](testing.md)),
  а по машинам — нет.
...
- **Прогон тестов оставляет за собой утечку:** «ObjectDB instances were leaked»
  и «resources still in use» в конце. Проверено 2026-09-14: дело не в тестах —
  то же печатает и обычный запуск игры на 300 кадров (2026-09-24: четыре
  экземпляра и два ресурса). Значит, что-то не освобождается при выходе. Проверки
  это не валит и в кадре не видно; разобрать, когда дойдут руки до сборки уровня.
- **Качание лампы после выстрела не заведено:** сбитая падает сразу (ADR-0007),
  подвес — шнур — есть (ADR-0023, «Чего в вехе нет»).
- **Четыре почти одинаковых ожидания «пока Otto не встанет на ноги»** разъехались
  по `screenshotter.gd` и трём тестам (`test_building_architecture`,
  `test_building_assembly`, `test_building_dress`), два из них байт в байт. Общий
  помощник напрашивается, но он трогает src и tests разом — отдельной задачей.
- GUT сообщает о доступной версии 9.7.1. Проверено 2026-09-24: на GitHub как latest
