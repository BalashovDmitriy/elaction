...
| **Состояние** | План вехи сделан, кадры сняты, набор зелёный. **Осталось: авторевью, `check.ps1`, PR** |
...
- **Сид 1 не повторялся, потому что здание жило по настенным часам.** Выпуск
  ...
  на отсчёт в физике; в `_process` осталась только картинка. Правило записано
...
- **Прогон бота после всего:** сиды 1–3 проходятся, смертей 4, 3 и 1 при пороге
  `DEATHS_ALLOWED` = 6, документы собраны все. До вехи было 3, 2 и 1 — агенты
  в кабинах достают дальше, и это видно числом.
...
- [x] Агенты ездят в кабинах пассажирами. Идёт последней: перемер прогона должен
      быть один, а не два
- [x] Долг: прогон с боем на сиде 1 воспроизводится — тест и `playthrough.gd`
      сходятся до шага
