...
| Статус | M15 — 3D-greybox: здание, лифты, Otto и агенты в трёхмерной сцене, бот проходит тридцать этажей |
...
отдельно от узлов сцены и переехали без правок. Вид сейчас — греев-бокс: коробки
вместо моделей и текстур, один источник света на этаж. Игровые объекты — Otto, агенты,
двери, кабина, лампы, выход — светятся сами и читаются на погашенном этаже.
...
- Свет: этаж светел, пока на нём горит лампа — она и есть единственный источник
  этажа. Сбили — этаж падает до общего тона здания и остаётся таким навсегда.
  Выстрел несёт вспышку. Столб света в шахте и город за окнами вернутся в M17 и M19.
- Актёры: Otto светлый, агент тёмный; присевший ниже, мёртвый лежит — коробка
  отыгрывает позу, которую выбирает то же правило, что выбирало спрайт. Модели
  и анимации — M16.
...
Играть, проходить проверки и собирать релиз можно только с этими двумя. Ниже — то,
что нужно инструментам спрайтового пайплайна ([ADR-0011](docs/adr/0011-asset-pipeline.md)).
С M15 игра спрайты не читает; инструменты остаются до M16, где Blender начнёт отдавать
модели вместо кадров, и до M20, где вычищается остальное:
...
Звук синтезируется кодом; спрайтовые генераторы остались от 2D и игрой не читаются:

```powershell
python tools/render_audio.py       # звуки и музыка
python tools/render_actors.py      # прежние кадры актёров из Blender (до M16)
python tools/render_env.py         # прежние спрайты окружения (до M20)
